...
class Koleksi_museum_pdf extends MY_Controller {
	...
	private function _DisplayPdfTitleCode($nama_museum) {
	...
	private function SetTabelTitleRegistrasi(){
	...
	private function setDataKoleksiRegistrasi($start =0, $end =0 ){
		$this->load->library('datetimemanipulation');
		$params =array(intval($start), intval($end));
		$type = $this->input->post('batas');
		$search_key = array("no_regmus"   => '%', "id_jenis"  => '%',
			"keaslian_koleksi" => '%', 
			"keutuhan" => '%', 
			"kondisi" => '%', 
			"no_registrasi" => '%',
		 ...
		 $search_sql = array( "keaslian_koleksi_null" => ' OR keaslian_koleksi IS NULL',
			"keutuhan_null" => ' OR keutuhan IS NULL ',
			"kondisi_null" => 'OR kondisi IS NULL ',
			"id_negara_null" => 'OR id_negara_asal IS NULL',
			'cb_query' => '');
			//"cb" => '',
		...
		*/
        ...
        if(!empty($search)) {
			...
			$search_sql = array( "keaslian_koleksi_null" => '',
					"keutuhan_null" => '',
					"kondisi_null" => '',
					"id_negara_null" => '',
					'cb_query' => $search_sql['cb_query']);
	...
	private function _DisplayPdfTitleFormatKatalog($nama_museum ='') {
        // detail museum
        
        return $tablealign;
    }
    private function DisplayPdfDataKatalog($museum, $start =0 , $end =0 , $obj_tcpdf) {
            ...
            foreach($datakoleksi as $result) {
				...
				if(!empty($logdata)):
					if(!empty($logdata['admin_name'])):
						$result['nama_pencatat'] =  $logdata['admin_name'];
					else:
						$result['nama_pencatat'] =  $logdata['nama_user'];
					endif;
					$result['tanggal_pencatatan_view'] = $this->datetimemanipulation->GetFullDate($logdata['tanggal']);	
				else:
