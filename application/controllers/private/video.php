...
class Video extends MY_Controller {
    ...
    public function process_add() {
        ...
        if ($this->notification->valid_input()) {
            ...
            if($this->videomodel->process_video_add($params)) {
                ...
                if($tipe == 'embed'):
                $this->notification->set_message("Data berhasil disimpan, Lanjutkan dengan MEMASUKAN SCRIPT EMBED VIDEO pada KOTAK EMBED SCRIPT!");
				else:
	...
	public function form() {
        ...
        $this->smarty->assign("data", $data);
			if($data['tipe'] == 'upload'):
					$view_form = 'upload';
					$this->smarty->assign("url_process", site_url("private/video/process/upload"));
			else:
					$view_form = 'embed';
	...
	public function process_uploadvideo() {
        // load library
        $this->load->library('notification');
		$this->load->library('uploader');
        $this->load->model('videomodel');
	...
	public function process_embedvideo() {
        // load library
        $this->load->model('videomodel');
        $this->load->library('notification');
		 $this->load->library("uploader");
