...
class Foto extends MY_Controller {
            ...
            if($this->fotomodel->process_album_add($params)) {
				$id_album = $this->db->insert_id();
				 if (!empty($_FILES['image_album']['tmp_name'])) {
						// set file attachment
						$_FILES['image_album']['name'] = $id_album.'_'.$_FILES['image_album']['name'];
						$this->uploader->set_file($_FILES['image_album']);
						...
						// direktori
						$dir = 'doc/album/image_album/' . $id_album . '/';
						// proses upload
						if ($this->uploader->upload_image($dir, 1000)) {
							$this->db->set("image_album",$this->uploader->get_file_name());
                ...
                $this->notification->set_message("Data berhasil disimpan, Lanjutkan dengan mengupload Foto");
    ...
    public function process_add_foto() {
        // load library
        $this->load->model('fotomodel');
        $this->load->library('notification');
		$this->load->library("uploader");
            ...
            if($this->fotomodel->process_foto_add($params)) {
				...
				$urutan = $this->input->post('urutan');
				 if (!empty($_FILES['foto']['tmp_name'])) {
						// set file attachment
						$_FILES['foto']['name'] = $id_album.'_'.$id_foto.'_'. $urutan .'_'. $_FILES['foto']['name'];
    ...
    public function process_edit() {
	 ...
	 public function process_edit_foto() {
        // load library
        $this->load->model('fotomodel');
        $this->load->library('notification');
		 $this->load->library("uploader");
    ...
    public function process_hapus() {
     ...
     public function process_hapus_foto() {
        // load library
        $this->load->library('notification');
        $this->load->library('uploader');
        $this->load->model('fotomodel');
    ...
    public function hapusgambarfoto() {
        // load library
        $this->load->library('notification');
		$this->load->library('uploader');
        $this->load->model('fotomodel');
