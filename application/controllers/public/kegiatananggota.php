...
class Kegiatananggota extends MY_Controller {
    ...
    public function index() {
       ...
       $this->smarty->assign('template_content',"web/agenda/list.html");
		...
		// get data
        $result = $this->agendamodel->get_list_agenda_anggota($params);
		...
		if(!empty($result)):
			foreach($result as $key=>$data):
				...
				$result[$key]['url_detail'] = site_url('public/kegiatananggota/detail/'.$data['id_agenda'].'/'.url_title($data['judul_english']));
				...
				$result[$key]['url_detail'] = site_url('public/kegiatananggota/detail/'.$data['id_agenda'].'/'.url_title($data['judul_agenda']));
		...
		if($this->act_lang == 'en'){
		$this->smarty->assign("page_modul", 'Member Event');
		}else{
		$this->smarty->assign("page_modul", 'Kegiatan Anggota');
		}
	...
	public function detail(){
		...
		if($this->act_lang  == 'en'){
		$this->smarty->assign("page_modul", 'Member Event');
		$this->smarty->assign("page_name", 'Member Event Details');
		}else{
		$this->smarty->assign("page_modul", 'Kegiatan Anggota');
		$this->smarty->assign("page_name", 'Detail Kegiatan Anggota');
		}
		$this->smarty->assign("page_modul_url", site_url('public/kegiatananggota'));
