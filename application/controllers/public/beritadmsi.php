<?php
	...
	class Beritadmsi extends MY_Controller {
		...
		function index() {
			// template content
			$this->smarty->assign('template_content',"web/beritadmsi/list.html");
			// load
			...
			if($this->act_lang  == 'en'){
				...
				$result[$key]['url_detail'] = site_url('public/beritadmsi/detail/'.$data['id_berita'].'/'.url_title($data['judul_english']));
		...
		public
		function detail(){
			$this->smarty->assign('template_content',"web/beritadmsi/detail.html");
			$this->load->model('beritamodel');
			...
			if($this->act_lang  == 'en'):
			$data['tanggal'] = $this->datetimemanipulation->GetFullDateWithDayEn($data['tanggal']); else :
			$data['tanggal'] = $this->datetimemanipulation->GetFullDateWithDay($data['tanggal']);
			endif;
			$pathdok = 'dok/'.$data['id_berita'].'/'.$data['file_berita'];
			
			if(is_file($pathdok)){
				$data['file_berita'] = site_url('public/beritadmsi/download/'.$data['id_berita'].'/'.url_title($data['file_berita']));
			} else {
				$data['file_berita']= '';
			}
			...
			if($this->act_lang == 'en'):
			$this->smarty->assign("page_modul", 'News');
			$this->smarty->assign("page_name", 'Detail News'); else :
			$this->smarty->assign("page_modul", 'Berita');
		...
		function download(){
			...
			// data
			$id_berita= $this->uri->segment(4,0);
			$judul_berita= $this->uri->segment(5,0);
			$data = $this->beritamodel->get_berita_by_id($id_berita);
