...
class Foto extends MY_Controller {
    ...
    public function index() {
		...
		// get data
        $result = $this->fotomodel->get_list_album_limit_public($params);
		$this->smarty->assign("total", $totaldata);
       	if(!empty($result)):
			...
			if($this->act_lang  == 'en'):
				...
				$path = 'doc/album/image_album/'.$data['id_album']."/";
				...
				*/
				if(is_file($path.$data['image_album'])){
					...
					$result[$key]['image_album'] = BASEURL.$path.$data['image_album'];
		...
		$this->smarty->assign("album_list", $result);
	...
	public function detail(){
		...
		if(!empty($nama_album)){
			...
			if($this->act_lang  == 'en'){
				$nama_album[$key]['nama_album'] = $nama['nama_english']; 
				}else{
