...
class Hargamodel extends CI_Model {
    ...
    function get_list_harga($param) {
        $sql = "SELECT * from harga_m where id_katharga = ? and tahun = ? and id_bulan = ? ";
    ...
    function get_list_harga_private() {
        $sql = "SELECT a.*, b.id_bulan, b.bulan, s.katharga FROM harga_m a LEFT JOIN katharga_m s ON a.id_katharga = s.id_katharga LEFT JOIN bulan_m b ON a.id_bulan = b.id_bulan
    ...
    function get_list_harga_domestik_tabel() {
        ...
        FROM harga_m WHERE id_katharga = '1' group by tahun";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            $result = $query->result_array();
            $query->free_result();
    ...
    function get_list_harga_ekspor_tabel() {
     ...
     function get_list_harga_grafik() {
        ...
        $sql = "SELECT a.id_harga, a.tahun , a.id_bulan, a.id_katharga ,
         a.harga_rupiah,  a.harga_dolar,  a.sumber, b.id_bulan, b.bulan, s.id_katharga, s.katharga
          FROM harga_m a 
            LEFT JOIN katharga_m s ON a.id_katharga = s.id_katharga 
            LEFT JOIN bulan_m b ON a.id_bulan = b.id_bulan WHERE  a.tahun = $tahunmax ORDER BY b.id_bulan ASC";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            $result = $query->result_array();
            $query->free_result();
    ...
    function get_list_harga_grafik_domestik() {
        ...
        $sql = "SELECT a.id_harga, a.tahun , a.id_bulan, a.id_katharga ,
         a.harga_rupiah,  a.harga_dolar,  a.sumber, b.id_bulan, b.bulan, s.id_katharga, s.katharga
          FROM harga_m a 
            LEFT JOIN katharga_m s ON a.id_katharga = s.id_katharga 
            LEFT JOIN bulan_m b ON a.id_bulan = b.id_bulan WHERE a.id_katharga = '1' and a.tahun = $tahunmax  ORDER BY b.id_bulan ASC";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            $result = $query->result_array();
            $query->free_result();
    ...
    function process_harga_add($params) {
        $sql = "INSERT INTO harga_m (id_katharga, tahun, id_bulan, harga_rupiah, harga_dolar, sumber)
                VALUES (?, ?, ?, ?, ?, ?)";
        return $this->db->query($sql, $params);
    }

    function process_harga_edit($params) {
        $sql = "UPDATE harga_m SET id_katharga = ? , tahun = ?, id_bulan = ?, harga_rupiah = ?, harga_dolar = ?, sumber = ? WHERE id_harga = ? ";
        return $this->db->query($sql, $params);
    }
