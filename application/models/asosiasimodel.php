...
class asosiasimodel extends CI_Model {
    ...
    function get_total_asosiasi_daerah($params) {
        $sql = "SELECT COUNT(*)'total' FROM asosiasi_m a
                INNER JOIN spt_kota_m b ON a.id_kota = b.id_kota
                WHERE b.id_propinsi = ? ";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
            $result = $query->row_array();
    ...
    function get_data_asosiasi($params) {
        $sql = "SELECT DISTINCT a.*, admin_name, nama_kota, nama_propinsi
                FROM asosiasi_m a
                LEFT JOIN administrator_m b ON a.mdb = b.id_user
                LEFT JOIN spt_kota_m c ON a.id_kota = c.id_kota
                LEFT JOIN spt_propinsi_m d ON c.id_propinsi = d.id_propinsi
                WHERE nama_asosiasi LIKE ? 
	...
	function get_data_asosiasi_all($params) {
        $sql = "SELECT DISTINCT a.*,nama_kota, nama_propinsi
                FROM asosiasi_m a
                LEFT JOIN spt_kota_m c ON a.id_kota = c.id_kota
                LEFT JOIN spt_propinsi_m d ON c.id_propinsi = d.id_propinsi
                WHERE c.id_propinsi LIKE ? 
	...
	function get_data_asosiasi_daerah_random($params) {
        $sql = "SELECT a.*,  nama_kota, nama_propinsi
                FROM asosiasi_m a
                LEFT JOIN spt_kota_m c ON a.id_kota = c.id_kota
                LEFT JOIN spt_propinsi_m d ON c.id_propinsi = d.id_propinsi
                WHERE a.foto_utama IS NOT NULL  
                ORDER BY RAND() LIMIT ?, ? ";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
            $result = $query->result_array();
    ...
    function get_asosiasi_by_id($id) {
        $sql = "SELECT DISTINCT a.*, nama_kota, d.id_propinsi, nama_propinsi 
                FROM asosiasi_m a
                LEFT JOIN spt_kota_m c ON a.id_kota = c.id_kota
                LEFT JOIN spt_propinsi_m d ON c.id_propinsi = d.id_propinsi
                WHERE a.id_asosiasi = ?";
    ...
    function process_asosiasi_add($params) {
        ...
        $sql = "INSERT INTO asosiasi_m (id_kota, 
    ...
    function process_asosiasi_edit($params) {
        ...
        $sql = "UPDATE asosiasi_m SET id_kota = ?,  nama_asosiasi = ?, profil = ?, profil_english = ?, alamat = ?, kodepos = ?, 
