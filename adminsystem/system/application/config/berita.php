...
class Berita extends MY_Controller {
				...
				$id_berita = $this->db->insert_id();
				 if (!empty($_FILES['image_berita']['tmp_name'])) {
					...
					if(!empty($_FILES['file']['name'])){
									...
									// set rules (kosongkan jika tidak menggunakan batasan sama sekali)
									$rules = array('allowed_filesize' => 100000000);
									$this->uploader->set_rules($rules);
									// direktori
									$dir = 'doc/berita/'. $id_berita.'/';
    ...
    public function edit() {
		...
		if(is_file($path.$data['image'])){
			...
			$data['image_berita'] = '<img src="'.BASEURL.$path.$data['image'].'" border="0"><br /><input type="button" value="Hapus Gambar" onClick="javascript:document.location=\''.$url_hapus.'\';">';
        ...
        if ($this->notification->valid_input()) {
			...
			if(is_array($params)):
				...
				foreach($params as $id):
					...
					$id_data = $id;
					$note_aksi = "Hapus Data Berita Oleh ".$this->user_data['admin_name']." dengan rincian, ID : ".$id_data."; Nama Judul : ".$berita['judul'].";";
					
					$paramslog = array(
						"nama_data" => "berita", 
						"aksi" => "delete", 
						"id_instansi" => $this->user_data['no_regmus'],
						"id_data" => $id_data,
